...
function sine(wavelength, amplitude, y) {
...
function createPaths(nWaves) {
  ...
  for (let i = 0; i < nWaves; ++i) {
    const swellDuration = 12 + ((i / nWaves * 1.5)**2 - 3 * i / nWaves) * 6;
    const swellDelay = -nWaves + i * 0.3
    const waveDuration = 15;
    const waveDelay = ((i / nWaves * 1.5)**2 - 3 * i / nWaves) * 6;
    paths.push(
...
const Waves = (props) => {
  ...
  useEffect(() => {
    setPaths(createPaths(props.nWaves));
  }, [props.nWaves]);
